...
if ( ! class_exists( 'Lafood_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Lafood_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'lafood' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'lafood' );
			$this->widget_id          = 'lafood-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				...
				'category' => array(
					...
					'label'            => esc_html__( 'Select category to show', 'lafood' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'lafood' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'lafood' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid'  => array(
							'label' => esc_html__( 'Grid', 'lafood' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'lafood' ),
						),
					),
					'label'   => esc_html__( 'Choose Layout Type', 'lafood' ),
				),
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'lafood' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$grid_class_array = apply_filters( 'lafood_taxonomy_tiles_widget_grid_class', array(
				...
				echo apply_filters( 'lafood_taxonomy_tiles_widget_before',
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
						...
						'sufix'   => _n_noop( '%s post', '%s posts', 'lafood' ),
				...
				echo apply_filters( 'lafood_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'lafood_register_taxonomy_tiles_widget' );
	/**
	 * Register taxonomy tiles widget.
	 */
	function lafood_register_taxonomy_tiles_widget() {
		register_widget( 'Lafood_Taxonomy_Tiles_Widget' );
	}
}

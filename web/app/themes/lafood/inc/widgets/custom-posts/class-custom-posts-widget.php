...
if ( ! class_exists( 'Lafood_Custom_Posts_Widget' ) ) {

	/**
	 * Class Lafood_Custom_Posts_Widget.
	 */
	class Lafood_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'lafood' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'lafood' );
			$this->widget_id          = apply_filters( 'lafood_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'lafood_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = lafood_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'lafood' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'lafood' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'lafood' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'lafood' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'lafood' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'lafood' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'lafood' ),
				),
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'lafood' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						'date'          => esc_html__( 'Date', 'lafood' ),
						'author'        => esc_html__( 'Author', 'lafood' ),
						'comment_count' => esc_html__( 'Comment count', 'lafood' ),
						'category'      => esc_html__( 'Category', 'lafood' ),
						'post_tag'      => esc_html__( 'Tag', 'lafood' ),
						'more_button'   => esc_html__( 'More Button', 'lafood' ),
					),
					'label'   => esc_html__( 'Display post meta data', 'lafood' ),
				),
				'button_text' => array(
					'type'  => 'text',
					'value' => 'Read More',
					'label' => esc_html__( 'Post read more button label', 'lafood' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'lafood_custom_posts_widget_view_dir', 'inc/widgets/custom-posts/views/custom-post-view.php' ), false, false );
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', lafood_theme()->customizer->get_default( 'footer_widget_columns' ) );
	...
	add_action( 'widgets_init', 'lafood_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function lafood_register_custom_posts_widget() {
		register_widget( 'Lafood_Custom_Posts_Widget' );
	}
}

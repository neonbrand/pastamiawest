 ...
 * @return array
 */
function lafood_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! lafood_is_meta_visible( $meta, $context ) ) {
		return;
	}
	 ...
	 * $6%s - `thumbnail`
	 */
	$defaults = apply_filters( 'lafood_default_args_share_buttons', array(
		'facebook' => array(
			'icon'      => 'fa fa-facebook',
			'name'      => esc_html__( 'Facebook', 'lafood' ),
			'share_url' => 'https://www.facebook.com/sharer/sharer.php?u=%3$s&t=%4$s',
		),
		'twitter' => array(
			'icon'      => 'fa fa-twitter',
			'name'      => esc_html__( 'Twitter', 'lafood' ),
			'share_url' => 'https://twitter.com/intent/tweet?url=%3$s&text=%4$s',
		),
		'google-plus' => array(
			'icon'      => 'fa fa-google-plus',
			'name'      => esc_html__( 'Google+', 'lafood' ),
			'share_url' => 'https://plus.google.com/share?url=%3$s',
		),
		'pinterest' => array(
			'icon'      => 'fa fa-pinterest-p',
			'name'      => esc_html__( 'Pinterest', 'lafood' ),
	...
	$default_config = apply_filters( 'lafood_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'lafood_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'lafood' ) . $network['name'],
 ...
 * @return string
 */
function lafood_get_single_share_buttons() {
	ob_start();
